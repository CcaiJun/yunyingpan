...
Yingpan 内置了企业级的数据一致性保障机制，确保在意外断电、网络中断或系统崩溃等极端场景下数据的完整性。

### 🛡️ 核心安全机制

1.  **索引原子更新 (Atomic Index Update)**
    - `block_index.json` 采用“先写入临时文件，再执行 MOVE 重命名”的原子操作。
    - 彻底杜绝因上传中断导致的索引文件损坏风险。

2.  **多版本时光机 (Index Versioning)**
    - 云端始终保留最近 5 个版本的索引快照（`block_index_{timestamp}.json`）。
    - 即便最新的索引因逻辑错误损坏，系统也能通过历史快照回滚，确保磁盘至少能以“几分钟前”的状态成功挂载。

3.  **启动自动自愈 (Auto-Healing)**
    - 磁盘挂载时会自动执行三级回滚策略：
        1. 尝试加载主索引 (`block_index.json`)。
        2. 失败则尝试加载指针文件 (`latest_index_pointer.json`) 指向的版本。
        3. 仍失败则扫描云端目录，自动回滚到最近一个完好的历史版本。

4.  **本地预写日志检测 (WAL & Dirty Check)**
    - 系统启动时会自动比对元数据数据库与本地缓存文件。
    - 若发现“元数据标记已写入但本地缓存文件丢失”（典型的断电特征），会自动生成 `DATA_LOSS_WARNING.txt` 告警报告，明确告知用户哪些数据块受到了影响。

### 🔄 云端还原与匹配

在新设备上安装 Yingpan 后，无需手动备份配置文件，即可直接从云端找回磁盘：

1.  在控制台首页点击“云端还原与匹配”。
2.  输入 WebDAV 账号信息，系统会自动扫描云端已存在的计算机目录。
3.  **智能状态识别**：
    - **🟢 配置完好**: 自动拉取云端 `config.json`，一键全量恢复（包含密码、压缩设置等）。
    - **🔵 仅有索引**: 若配置丢失但数据索引还在，系统会引导您手动创建同名磁盘，自动对接原有数据。
    - **🟠 仅有数据**: 对于极端的孤儿数据文件夹，提供手动挂载入口。
